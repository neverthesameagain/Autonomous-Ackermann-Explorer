...
class AStarPlanner:
    ...
    def plan(self, start_x: float, start_y: float,
        ...
        # Fallback if goal cell blocked
        if inflated[goal_gy, goal_gx]:
            print("⚠️ Goal cell blocked after inflation → reducing inflation radius...")
            for r in range(inflation_radius - 1, -1, -1):
                inflated = self._inflate_obstacles(r)
                if not inflated[goal_gy, goal_gx]:
                    print(f"  ✓ Recovered with inflation={r}")
                    self._inflated_mask = inflated.copy()
                    break
            else:
                print("✗ Goal cell still blocked → aborting.")
                return None
